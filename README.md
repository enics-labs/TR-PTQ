...
| `run_glue_quant.py` | Generic configurable GLUE runner for MRPC and CoLA. |
...
Use `run_glue_quant.py` for task-configurable MRPC or CoLA runs:

```bash
GLUE_QUANT_CONFIG=quant_config_cola.json python run_glue_quant.py
```

Set `task_name` to `mrpc` or `cola`. The runner chooses the matching custom head, tokenizer fields, GLUE split, metric, calibration flow, and timestamped JSON output.
...
| `task_name` | string | `mrpc` | Task used by `run_glue_quant.py`; valid values are `mrpc` and `cola`. |
